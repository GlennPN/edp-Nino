            ...
            @forelse($applicants as $applicant)
                <tr>
                    ...
                    <td class="text-center px-1">
                        <a href="{{ route('client.applicants.show', $applicant->id) }}" class="btn btn-sm btn-primary me-1" style="color: white; text-shadow: 2px 2px 4px #000000;">View</a>
                        <a href="{{ route('client.applicants.edit', $applicant->id) }}" class="btn btn-sm btn-warning me-1" style="color: white; text-shadow: 2px 2px 4px #000000;">Edit</a>
                        <form action="{{ route('client.applicants.destroy', $applicant->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-sm btn-danger" style="color: white; text-shadow: 2px 2px 4px #000000;" onclick="return confirm('Delete this applicant?')">Delete</button>
                        </form>
                    </td>
                </tr>
            @empty
