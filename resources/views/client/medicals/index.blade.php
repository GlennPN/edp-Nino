                ...
                @forelse($medicals as $medical)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{ route('client.medicals.edit', $medical->id) }}" class="btn btn-sm btn-secondary" style="color: white; text-shadow: 2px 2px 4px #000000;">Edit</a>
                            <form action="{{ route('client.medicals.destroy', $medical->id) }}" method="POST" class="d-inline">
                                @csrf @method('DELETE')
                                <button onclick="return confirm('Delete this record?')" class="btn btn-sm btn-danger" style="color: white; text-shadow: 2px 2px 4px #000000;">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
