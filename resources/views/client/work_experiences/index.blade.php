                ...
                @forelse($experiences as $exp)
                    <tr>
                        ...
                        <td class="text-center px-1">
                            <a href="{{ route('client.work_experiences.edit', $exp->id) }}" class="btn btn-sm btn-secondary me-1" style="color: white; text-shadow: 2px 2px 4px #000000;">Edit</a>
                            <form action="{{ route('client.work_experiences.destroy', $exp->id) }}" method="POST" class="d-inline">
                                @csrf @method('DELETE')
                                <button onclick="return confirm('Delete this?')" class="btn btn-sm btn-danger" style="color: white; text-shadow: 2px 2px 4px #000000;">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
