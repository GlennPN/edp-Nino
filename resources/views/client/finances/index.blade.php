                ...
                @forelse($finances as $finance)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{ route('client.finances.edit', $finance->id) }}" class="btn btn-sm btn-secondary" style="color: white; text-shadow: 2px 2px 4px #000000;">Edit</a>
                            <form action="{{ route('client.finances.destroy', $finance->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-danger" style="color: white; text-shadow: 2px 2px 4px #000000;" onclick="return confirm('Delete this record?')">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
